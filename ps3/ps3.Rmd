...
```{r packages, results='hide', message=FALSE, echo=F, eval=T, cache=cache_chunks}
...
# reference category = Wednesday
reg_fig1_na = lm(NOx ~ day_of_week, data=df)
# NOx_hat = constant plus the regression residuals
df_daily_sums = df %>% 
    mutate(NOx_hat = ifelse(is.na(NOx), NA, reg_fig1_na$coefficients['(Intercept)'] + reg_fig1_na$residuals)) %>%
...
```{r Figure1, echo=F, fig.height = 4, fig.width = 7,  fig.cap = "Total Daily NO$\\_{\\text{x}}$ Emissions in the NBP-Participating States"}
